...
import styles from "./SsnLast4.module.scss";
import styles3 from "./SsnLast4.module.css";
...
import LastSSNTable from "../../../components/services-pages/ui/table/LastSSNTable.jsx";
import ButtonClassicTwo from "../../../components/ui/button/button-classic-two/ButtonClassicTwo.jsx";
import usePrices from "../../../hooks/usePrices.js";
import {post_last_ssn, post_last_ssn_history} from "../../../app/api/protected.api.js";
...
const SsnLast4 = () => {
    ...
    function search() {
        if (FirstName.trim() !== '' && LastName.trim() !== '' && Address.trim() !== '' && City.trim() !== '' && State.trim() !== '') {
            ...
            post_last_ssn(FirstName, LastName, Address, City, State).then((r) => {
                if (r.status === 200) {
                    setToken(r.data)
                } else {
                    setPostBGError(true)
    ...
    return (
        <div className={`fade_in ${styles.block}`}>
            <div className={styles.header}>
                <div className={styles.title} style={{marginBottom: 32}}>SSN Last 4</div>
            </div>
            <>
                ...
                <SearchButton disabled={price_loading} clickHandler={() => search()}
                              price={price_loading ? 'Loading...' : `${prices.ssn_last} $`}
