...
import styles3 from "./dob.module.css";
...
import DOBTable from "../../../components/services-pages/ui/table/DOBTable.jsx";
import usePrices from "../../../hooks/usePrices.js";
import {post_dob, post_dob_history} from "../../../app/api/protected.api.js";
...
const Dob = () => {
    ...
    function search() {
        if (FirstName.trim() !== '' && LastName.trim() !== '' && Address.trim() !== '' && City.trim() !== '' && State.trim() !== '') {
            ...
            setToken('')
            post_dob(FirstName, LastName, Address, City, State).then((r) => {
                if (r.status === 200) {
                    setToken(r.data)
                } else {
    ...
    return (
        <div className={`fade_in ${styles.block}`}>
            <div className={styles.header}>
                <div className={styles.title} style={{marginBottom: 32}}>Dob</div>
            </div>
            <>
                ...
                <SearchButton disabled={price_loading} clickHandler={() => search()}
                              price={price_loading ? 'Loading...' : `${prices.dob} $`}
